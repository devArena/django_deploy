...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
 config.vm.define :djangovm do |django_config|
  ...
  # Enable provisioning with chef solo, specifying a cookboks path (relative
  # to this Vagrantfile), and some recipes and/or roles.
  #
  django_config.vm.provision :chef_solo do |chef|
    ...
    chef.json = { 
      :nginx =>{ :install_method => "package" },
      :django_deploy =>{ :username => "postgres"},
      "postgresql" => {
        "password" => {
          "postgres" => "test1234"
          }
